# 以配置/文档为主，辅以少量注册类代码修改的内容

本文件说明哪些功能主要可以通过修改配置或文档（SOUL.md、配置文件、说明性文本、模板文件）来实现，哪些地方还需要做**少量注册类代码修改**，以及哪些能力虽然可以先准备配置文件，但功能本身仍需后续代码实现。

## 1. Agent 定义（SOUL.md 文件）

### 1.1 现有的 Agent（无需新增目录）
...
### 1.2 需要新增的 Agent（新增目录，但仍以配置/文档为主）

虽然需要创建新目录，但这些仍主要属于配置/文档层面的工作：
...
## 2. 看板与组织注册（需少量 server.py 注册类修改）

这部分**不是框架重写**，但也不应再表述为“纯配置”。它属于少量注册类代码修改，主要目的是让新部门/新 Agent 能在看板与组织映射中被正确识别。

### 2.1 看板显示配置：`_AGENT_DEPTS`

现有项目在 `dashboard/server.py` 中有 `_AGENT_DEPTS` 配置，需要修改这个数组：
...
### 2.2 组织映射配置：`_ORG_AGENT_MAP`

除了 `_AGENT_DEPTS`，还应补充组织到 Agent 的映射。否则看板能显示，路由和组织识别仍可能不完整。
...
新设计中的所有模板都可以作为独立文件存放，不需要改动代码框架：
...
以下文件可以先定义为配置或数据文件，但要区分：
- **立即可用的配置**：只要项目现有逻辑会读取它，就能马上生效。
- **配置占位 / schema**：文件可以先设计出来，但功能本身仍需后续代码或调度逻辑实现。

| 配置项 | 文件位置 | 当前性质 | 说明 |
|-------|---------|---------|------|
| Agent 配置 | `data/agent_config.json` | 立即可用配置 | 可配置各 Agent 的模型、技能等 |
| 定时任务表 | `data/scheduled_tasks.json`（需新增） | 配置占位 / schema | 可先定义任务结构，但真正的 Cron 调度与执行链路属于后续实现 |
| 配额配置 | `data/quota_config.json`（需新增） | 配置占位 / schema | 可先定义阈值和策略，但真正的配额查询、节流决策、执行逻辑属于后续实现 |
| 国策库数据 | `data/guoceluku.json`（需新增） | 配置占位 / 数据存储 | 可先定义议题字段和存储结构，但真正的入池、挂载汇报、状态流转属于后续实现 |
...
2. ✅ 新增 Agent 目录与 SOUL.md
3. ✅ 所有输出模板文件
4. ✅ 各类配置数据文件的 schema 与初始内容

### 5.2 需要少量注册类代码修改的部分

1. ⚠️ `dashboard/server.py` 中的 `_AGENT_DEPTS`
2. ⚠️ `dashboard/server.py` 中的 `_ORG_AGENT_MAP`
...
5. ❌ Cron 定时调度本身
6. ❌ 配额查询与节流决策逻辑本身
7. ❌ 国策库入池、挂载汇报与状态流转逻辑本身

一句话概括：
**这份清单应该理解为“配置优先 + 少量注册代码 + 为后续功能预留 schema”，而不是“只要把文件写出来，所有能力就已经存在”。**
